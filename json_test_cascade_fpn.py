...
from model.roi_layers import nms,soft_nms
...
categoryList={'bg':0,'pre-twisted suspension clamp':1,'bag-type suspension clamp':2,'compression-type strain clamp':3,'wedge-type strain clamp':4,'hanging board':5,'u-type hanging ring':6,'yoke plate':7,'parallel groove clamp':8,'shockproof hammer':9,'spacer':10,'grading ring':11,'shielded ring':12,'weight':13,'adjusting board':14}
...
def parse_args():
  ...
  parser.add_argument('--dataset', dest='dataset',
                      ...
                      default='pascal_voc_cap', type=str)
...
if __name__ == '__main__':
  ...
  load_name = os.path.join(input_dir,
    'cascade_fpn_{}_{}_{}.pth'.format(args.checksession, args.checkepoch, args.checkpoint))

  pascal_classes = np.asarray(['__background__',
                      'pre-twisted suspension clamp', 'bag-type suspension clamp', 'compression-type strain clamp',
  ...
  while (num_images >= 0):
      ...
      for j in xrange(1, len(pascal_classes)):
          ...
          if inds.numel() > 0:
            ...
            # soft_nms(dets, box_scores, sigma=0.5, thresh=0.001, cuda=0):
            # keep = soft_nms(cls_boxes[order, :], cls_scores[order],thresh=0.5,cuda=args.cuda)
...
#             print(pascal_classes[j],cls_dets.cpu())
      ...
      if vis and webcam_num == -1:
          ...
          # cv2.waitKey(0)
          result_path = os.path.join("images_cap", imglist[num_images][:-4] + "_det.jpg")
