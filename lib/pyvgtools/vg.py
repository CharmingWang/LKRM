...
class VG:
    def __init__(self, data_dir, annotation_file=None, num=-1, stats=False, align_dir=None):
        ...
        if annotation_file is not None:
            ...
            tic = time.time()
            if annotation_file == 'detections_val_results.json':
                dataset = json.load(open(os.path.join(self.data_dir,annotation_file), 'r'))
            elif annotation_file == 'detections_test_results.json':
                dataset = json.load(open(os.path.join(self.data_dir, annotation_file), 'r'))
            else:
                dataset = json.load(open(os.path.join(annotation_file), 'r'))  # 将object.json信息读取到内存中，在dataset中存储起来
    ...
    def create_index(self):
        ...
        if self.num < 0:
            self.num = len(self.dataset)#87946
            ...
            for ann in img['objects']:
                ...
                # only consider those objects with exactly one synset
                    ...
                    if 'category_id' not in ann:
                        ...
                        self.cats[category_id] = synset#将类别信息存储到cats中
