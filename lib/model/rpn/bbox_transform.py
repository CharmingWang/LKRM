...
def bbox_decode(rois, bbox_pred, batch_size, class_agnostic, classes, im_info, training, cls_prob):
    ...
    if cfg.TEST.BBOX_REG:
        ...
        box_deltas = bbox_pred.data# [1,128,4]
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            ...
            if class_agnostic or training:
                ...
                cls_prob[:,0]=0
                bbox_pred_cls_argmax=torch.argmax(cls_prob,dim=1)# [300]
                # print(bbox_pred_cls_argmax)
                for i in range(bbox_pred.size(1)):
                    bbox_pred_cls_argmax[i]=bbox_pred_cls_argmax[i]+i*classes

                bbox_pred_max=bbox_pred.view(batch_size,-1,4)# [1,4500,4]
                bbox_pred_max=torch.index_select(bbox_pred_max,1,bbox_pred_cls_argmax)# [1,300,4]
        ...
        pred_boxes = bbox_transform_inv(boxes, box_deltas, batch_size)# [1,128,4]
