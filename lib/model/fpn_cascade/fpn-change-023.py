...
class _FPN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, cls_a_prob, cls_r_prob, cls_s_prob,cls_s_ac_prob,cls_s_d_prob,cls_s_i_prob, modules_size, modules_exist, hkrms_exist):
        ...
        if self.modules_exist[2]:
            ...
            self.appearance_transform = nn.Linear(15, 2048)
        self.sc_forward_fc1 = nn.Linear(2048, 10)
        self.sc_forward_fc2 = nn.Linear(10, 2048)
        self.u_matrix = nn.Linear(12, 1, bias=False)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        if True:
            ...
            if self.hkrms_exist[1] == 1:
                ...
                if self.modules_exist[2] == 3:
                    ...
                    PE_normal = F.softmax(PE)
                    # appearance_feat_b3 = self.appearance_transform(cls_pro_1st)
                    ...
                    adjs_inter = adjs_inter.view(n_boxes, n_boxes)
                    adjs_inter_normal = F.softmax(adjs_inter)
                    Adj_relation = PE_normal.mul(adjs_inter_normal)
                    Adj_relation_new = F.softmax(Adj_relation,1)
                    for b in range(batch_size):  # 取每个batch单独进行操作
                        # adjs_inter_single = adjs_inter[b, :, :].clone()
                        # adjs_inter_single_new = adjs_inter[b, :, :].clone()
                        adjs_inter_single_new = F.normalize(Adj_relation_new, p=2, dim=1)
                        adjs_inter_single_new2 = adjs_inter_single_new.clone()
                        for i in range(len(adjs_inter_single_new[0])):
                            ...
                            adjs_inter_single_new2[i] = torch.where(adjs_inter_single_new[i] > list_adjs[top], one, zero)
        ...
        if self.training:
            return rois, cls_prob_3rd_avg, bbox_pred_2nd, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, RCNN_loss_cls_2nd, RCNN_loss_bbox_2nd, RCNN_loss_cls_3rd, RCNN_loss_bbox_3rd, rois_label, scence_lable
