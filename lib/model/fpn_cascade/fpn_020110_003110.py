            ...
            if self.modules_exist[1] == 2:
                ...
                if  self.sc_net_type_flag[1] == 1:
                    ...
                    if self.training:  # 训练时128
                        scence_feat = scence_feat.repeat(128, 1)
                    else:  # 测试时300，简单改法，后续改为标准型
                        scence_feat = scence_feat.repeat(300, 1)
                    seq_len = 2
                    h1 = cls_pooled_feat
                    for i in range(seq_len):
                        h1 = self.gru(scence_feat, h1)
                ...
                if self.modules_exist[2] == 3:
                    ...
                    for b in range(batch_size):  # 取每个batch单独进行操作
                        ...
                        for i in range(len(adjs_inter_single[0])):
                            ...
                            adjs_inter_single_new2[i] = torch.where(adjs_inter_single[i] > list_adjs[top], one, zero)
