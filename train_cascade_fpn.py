...
if __name__ == '__main__':
    ...
    output_dir = args.save_dir + "/" + args.net + "/" + args.dataset + "/" + str(args.lr) + "_" + str(
        args.lr_decay_step) + "_" + str(args.lr_decay_gamma) + "_" + args.hkrm_mode + args.cascade_mode + "/"
    ...
    sc_roidb = Variable(sc_roidb)

    if args.cuda:
        cfg.CUDA = True


    rela = int(args.hkrm_mode[0])
    attr = int(args.hkrm_mode[1])
    spat = int(args.hkrm_mode[2])
    module_exist = [rela, attr, spat]
    module_size = int(args.hkrm_size)
    hkrm1 = int(args.cascade_mode[0])
    hkrm2 = int(args.cascade_mode[1])
    hkrm3 = int(args.cascade_mode[2])
    ...
    if args.hkrm_mode != '000' and args.cascade_mode != '000':
        load_name = os.path.join(args.save_dir, '{}_{}.pth'.format(args.dataset, 'cascade'))
        print("loading initial baseline model %s" % (load_name))
        checkpoint = torch.load(load_name)
        # print(checkpoint['model'].keys())

        for key in list(checkpoint['model'].keys()):
            if key.find('spat_bbox_fully_connect') >= 0 or key.find('spat_bbox_fully_connect') >= 0 or key.find('occu_cls_fully_connect') >= 0 or key.find('occu_cls_fully_connect') >=0 \
                    or key.find('RCNN_cls_score_2nd.weight') >=0 or key.find('RCNN_bbox_pred_2nd')>=0:
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step, data in enumerate(dataloader, 0):
            with torch.no_grad():
                  ...
                  sc_roidbn = data[4]
            # image = im_data.data[0].cpu().numpy().transpose(1,2,0)
            ...
            if args.cascade:
                ...
                #        + RCNN_loss_cls_3rd.mean() + RCNN_loss_bbox_3rd.mean() + adja3_loss.mean() + adjr3_loss.mean() + adjs3_loss.mean()
                loss = rpn_loss_cls + rpn_loss_box \
                       + RCNN_loss_cls + RCNN_loss_bbox\
                       + RCNN_loss_cls_2nd + RCNN_loss_bbox_2nd \
                       + RCNN_loss_cls_3rd + RCNN_loss_bbox_3rd
            ...
            loss_temp += loss.item()
            #写入训练过程中得到的场景信息标签
            # if args.hkrm_mode == '020' and args.cascade_mode == '110':
            #     out_result = 'epoch：' + str(epoch) + ' name: '  +str(sc_roidbn)+ ' scence_lable: ' + str(scence_lable)
            #     f = open("sc_lable.txt", "a+")
            #     f.write(out_result + "\r\n")
            #     f.close()
            ...
            if step % args.disp_interval == 0:
                ...
                if args.mGPUs:
                    ...
                    if args.cascade:
                        ...
                        # loss_adja3 = adja3_loss.mean().item()
                        # loss_adjr3 = adjr3_loss.mean().item()
                        # loss_adjs3 = adjs3_loss.mean().item()
                        loss_rcnn_cls_2nd = RCNN_loss_cls_2nd
                        loss_rcnn_box_2nd = RCNN_loss_bbox_2nd
                        loss_rcnn_cls_3rd = RCNN_loss_cls_3rd
                        loss_rcnn_box_3rd = RCNN_loss_bbox_3rd
